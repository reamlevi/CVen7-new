...
parser.add_argument('-sl',dest='sLink',help='Enter the link for your social account')
...
if (result.username and result.password and (result.register)=='c'):
    ...
    if (result.name and result.education and result.pic and result.id and result.exp):
        Candidate.CreateCV(result.name,result.education,result.pic,result.id,result.exp,result.sLink)
